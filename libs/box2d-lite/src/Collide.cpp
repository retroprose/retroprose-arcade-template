...
int ClipSegmentToLine(ClipVertex vOut[2], ClipVertex vIn[2],
					  const Vec2& normal, Scaler offset, char clipEdge)
{
	// Start with no output points
	int numOut = 0;

	// Calculate the distance of end points to the line
	Scaler distance0 = Dot(normal, vIn[0].v) - offset;
	Scaler distance1 = Dot(normal, vIn[1].v) - offset;
	...
	if (distance0 * distance1 < 0.0f)
	{
		...
		Scaler interp = distance0 / (distance0 - distance1);
...
{
	// Setup
	Vec2 hA = Scaler(0.5f) * bodyA->width;
	Vec2 hB = Scaler(0.5f) * bodyB->width;
	...
	const Scaler relativeTol = Scaler(0.95f);
	const Scaler absoluteTol = Scaler(0.01f);
	...
	case FACE_A_X:
		{
			...
			Scaler side = Dot(posA, sideNormal);
			negSide = -side + hA.y;
			posSide =  side + hA.y;
			negEdge = EDGE3;
			posEdge = EDGE1;
	...
	case FACE_B_X:
		{
			...
			Scaler side = Dot(posB, sideNormal);
			negSide = -side + hB.y;
			posSide =  side + hB.y;
			negEdge = EDGE3;
			posEdge = EDGE1;
	...
	for (int i = 0; i < 2; ++i)
	{
		Scaler separation = Dot(frontNormal, clipPoints2[i].v) - front;
